...
@Log public class WorkerDAO implements DAO<Worker> {
    ...
    private static final String QUERY_ORDER_BY = "select * from trabajador order by desde";
    ...
    public Worker update(Worker worker) {
        ...
        try (PreparedStatement st = JDBCUtils.getConn().prepareStatement(UPDATE_BY_ID)) {
            ...
            if (rowsAffected > 0) {
                out = worker;
                log.info("Trabajador actualizado con exito.");
            } else {
     ...
     * @return
     */
    public static List<Worker> getAllOrderByFrom(){
        ...
        try (Statement st = JDBCUtils.getConn().createStatement()){
            ...
            log.info("Trabajadores añadidos con exito");
        } catch (SQLException e) {
            log.severe("Error al rellenar los trabajadores");
            throw new RuntimeException(e);
        }
